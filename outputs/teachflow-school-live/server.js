...
async function handleSessionApi(request, response, requestUrl) {
  ...
  if (request.method === "GET" && route === "/api/session/invite") {
    const invite = workspaceStore.getInviteByToken(requestUrl.searchParams.get("token"));
    if (!invite) {
  ...
  if (request.method === "POST" && route === "/api/session/register-teacher") {
    const registration = workspaceStore.registerTeacher(await readJsonBody(request));
    const session = sessionStore.createSession(registration.state, {
      accountId: registration.account.id,
      classId: registration.classRecord.id
    });
    sendJson(response, 201, {
      ...sessionStore.publicSession(session),
      invite: withInviteUrl(registration.invite, request)
    }, {
      "Set-Cookie": sessionStore.sessionCookie(session)
    });
    return;
  ...
  if (request.method === "POST" && route === "/api/session/register-student") {
    const registration = workspaceStore.registerStudentWithInvite(await readJsonBody(request));
    const session = sessionStore.createSession(registration.state, {
      accountId: registration.account.id,
      classId: registration.classRecord.id
    });
...
async function handleWorkspaceApi(request, response, requestUrl) {
  ...
  if (request.method === "POST" && route === "/api/workspace/class-settings") {
    sendJson(response, 200, workspaceStore.updateClassSettings(withContext(await readJsonBody(request), queryContext)));
    return;
  }

  if (request.method === "POST" && route === "/api/workspace/material-drafts") {
    sendJson(response, 200, workspaceStore.updateMaterialDraft(withContext(await readJsonBody(request), queryContext)));
    return;
  }

  if (request.method === "POST" && route === "/api/workspace/material-drafts/publish") {
    sendJson(response, 200, workspaceStore.publishMaterialDraft(withContext(await readJsonBody(request), queryContext)));
    return;
  }
...
async function handleStudentAgentApi(request, response, requestUrl) {
  ...
  if (request.method === "POST" && route === "/api/student-agent/wellbeing-chat") {
    sendJson(response, 200, await workspaceStore.answerStudentWellbeingChatLive(withContext(await readJsonBody(request), session.context)));
    return;
  }
...
async function handleAiApi(request, response, requestUrl) {
  ...
  if (request.method === "POST" && route === "/api/ai/student-wellbeing") {
    sendJson(response, 200, await workspaceStore.answerStudentWellbeingChatLive(withContext(await readJsonBody(request), session.context)));
    return;
  }
...
function withInviteUrl(invite, request) {
  return {
    ...(invite || {}),
