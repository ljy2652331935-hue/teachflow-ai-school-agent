...
assert.strictEqual(setupState.priorities[0].action, "RunningMisconceptionDiagnosis");

const pilotState = schoolAgentEngine.evaluateSystemState({workspace: {class: {name: "Physics 101"}, topic: {title: "Fourier Transform"}},
 analysis,
 intervention,
...
assert.ok(pilotState.guardrails.some((item) => item.includes("not a grade")));
assert.ok(pilotState.priorities.length >= 1);

const brief = schoolAgentEngine.buildMorningBrief({workspace: {class: {name: "Physics 101"}, topic: {title: "Fourier Transform"}},
 analysis,
 intervention,
 intervention_status: "published",
 student_memories: memories,
 ...
 understanding_map_nodes: nodes,
 export_packages: [{export_id: "export-demo"}]});
assert.ok(brief.includes("TeachFlow Agent brief"));
assert.ok(brief.includes("highest priority"));
