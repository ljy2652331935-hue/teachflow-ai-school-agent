...
const assignments = studentPortalEngine.createassignments({intervention,
...
const reflection = studentPortalEngine.createReflection({topic_id: "topic-fourier",
 student_alias: "S002",
 prompt: "Explain this concept in your own words.",
 response: "It is another way to represent the same signal."});
const attempt = studentPortalEngine.createMicroQuizAttempt({intervention_id: intervention.intervention_id,
 topic_id: "topic-fourier",
 student_alias: "S002",
 answers: [{question_id: "question-1", answer: "It finds frequency components in the signal."}]});
const stuckSignal = understandingMapEngine.createStuckSignal({topic_id: "topic-fourier",
 student_alias: "S002",
 stuck_type: "diagram",
 free_text: "I do not know which side is time domain."});

const nodes = understandingMapEngine.createUnderstandingMap({student_memories: memories,
 analysis,
 intervention,
...
assert.ok(nodes.some((node) => node.evidence.some((item) => item.includes("latest reflection"))));

const summary = understandingMapEngine.summariseclassMap({nodes,
 stuck_signals: [stuckSignal]});
...
const alternate = understandingMapEngine.createAlternateExplanation({memory: memories.find((memory) => memory.student_alias === "S002"),
 assigned_material: assignedmaterial,
 latest_stuck_signal: stuckSignal});
assert.strictEqual(alternate.student_alias, "S002");
assert.ok(alternate.explanation.includes("diagram"));
assert.ok(alternate.next_prompt.includes("one sentence"));
