...
async function generateImage(options) {
  ...
  for (const model of models) {
    try {
      const result = await callImagesApi({ ...options, model });
      return {
        ok: true,
        mode: "live",
        provider: "openai",
...
function imageFallbackModelName() {
  return process.env.OPENAI_IMAGE_FALLBACK_MODEL || DEFAULT_IMAGE_FALLBACK_MODEL;
}
