...
(async () => {try {await waitForServer();
 ...
 const teacherRegistration = await request("POST", "/api/session/register-teacher", {displayName: "System Test teacher",
 ...
 const firstStudentRegistration = await request("POST", "/api/session/register-student", {inviteToken,
 ...
 const secondStudentRegistration = await request("POST", "/api/session/register-student", {inviteToken,
 ...
 await request("POST", "/api/workspace/questions", {studentAlias: "S001",
 text: questionText}, firstStudentCookie);

 const privateCheckInMarker = `PRIVATE CHECKIN ${Date.now()}`;
 await request("POST", "/api/workspace/check-ins", {studentAlias: "S001",
 state: "frustrated",
 note: privateCheckInMarker,
 shareChoice: "private"}, firstStudentCookie);
 ...
 const wellbeingChat = await request("POST", "/api/student-agent/wellbeing-chat", {message: `I feel overwhelmed by science class. ${wellbeingMarker}`}, firstStudentCookie);
 ...
 shareChoice: "teacher_summary"}, firstStudentCookie);

 const studentAgentChat = await request("POST", "/api/student-agent/chat", {question: "Can you explain electrons with a simple picture idea?"}, firstStudentCookie);
 ...
 await request("POST", "/api/student-agent/share-to-teacher", {stuckType: "definition_gap",
 text: questionText}, firstStudentCookie);

 await request("POST", "/api/workspace/assignment", {studentAlias: "S001",
 status: "submitted"}, firstStudentCookie);

 await request("POST", "/api/workspace/messages", {studentAlias: "S001",
 text: "teacher, I need a smaller step about electrons and current."}, firstStudentCookie);
 ...
 const teacherAgentAnswer = await request("POST", "/api/ai/teacher-agent", {question: "Which student should I support first and why?"}, teacherCookie);
 assert.strictEqual(teacherAgentAnswer.body.agent, "teacher-agent");
 assert.ok(teacherAgentAnswer.body.answer);

 const teacherActionState = await request("POST", "/api/teacher-agent/actions", {type: "send_message",
 studentAlias: "S001",
 title: "Small electron step",
 text: "Start with this: an electron is a tiny charged particle. We will connect it to current one step at a time."}, teacherCookie);
 ...
 await request("POST", "/api/workspace/teacher-actions/read", {studentAlias: "S001",
 actionId: teacherAction.id}, firstStudentCookie);

 await request("POST", "/api/workspace/stuck-signals", {studentAlias: "S001",
 stuckType: "definition_gap",
 note: "I still need a smaller step after the teacher message.",
 linkedteacherActionId: teacherAction.id,
 responseType: "still_stuck"}, firstStudentCookie);
 ...
 try {await request("GET", "/api/teacher-agent/briefing", null, firstStudentCookie);} catch (error) {studentteacherAgentBlocked = /HTTP 403/.test(error.message);}
 assert.strictEqual(studentteacherAgentBlocked, true);

 let teacherWellbeingBlocked = false;
 try {await request("POST", "/api/student-agent/wellbeing-chat", {message: "teacher should not use the student wellbeing coach."}, teacherCookie);} catch (error) {teacherWellbeingBlocked = /HTTP 403/.test(error.message);}
 ...
 console.log("school live system integrity tests passed");} finally {child.kill();
...
function sourceCount(items, type) {const item = (items || []).find((entry) => entry.type === type);
...
function cookieFrom(response) {const setCookie = response.headers["set-cookie"];
