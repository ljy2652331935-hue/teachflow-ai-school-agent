...
const intervention = interventionEngine.generateIntervention({
  analysis,
  learning_objectives: [
    "Explain Fourier Transform in simple language.",
...
assert.ok(intervention.visual_aid.image_prompt.includes("复杂波形"));
assert.ok(intervention.micro_quiz.length >= 4);
assert.ok(intervention.student_facing_material.body.includes("两种表示"));
assert.ok(intervention.export_markdown.includes("修订后的教学计划"));
assert.ok(intervention.export_markdown.includes("学生讲义"));
