(function (global) {
  ...
  function defaultState() {
    const students = [...clone(defaultStudents), ...multiClassDemoStudents()];
    return {
      version: 1,
      school: {
        id: "school-demo",
      ...
      classes: demoClasses(students),
      className: "高二物理 A 班",
      topic: "傅里叶变换",
      students,
      assignments: demoAssignments(),
      questions: demoQuestions(),
      stuckSignals: demoStuckSignals(),
      checkIns: demoCheckIns(),
      messages: demoMessages(),
      teacherAgentActions: [],
      auditEvents: demoAuditEvents(),
      approvedMaterials: ["学生讲义", "视觉讲解图"],
      draftMaterials: ["课堂 PPT", "分层练习", "微型测试"],
      updatedAt: null
    };
  }
  ...
  function normalizeContext(context) {
    ...
    const accountFallback = defaultAccounts.find((account) => account.role === role) || defaultAccounts[0];
    const userId = requested.userId || requested.accountId || accountFallback.id;
  ...
  function normalizeState(state) {
    ...
    normalized.classes = normalized.classes.map((item) => ({
      ...item,
      studentAliases: Array.isArray(item.studentAliases) && item.studentAliases.length
        ? item.studentAliases
