(function (global) {
  function createAssignments(input) {
    ...
    return memories.map((memory) => ({
      id: `assignment-${intervention.intervention_id}-${memory.student_alias}`,
  ...
  function getAssignedMaterial(input) {
  ...
  function createReflection(input) {
    return {
      id: `reflection-${Date.now()}-${input.student_alias}`,
  ...
  function createMicroQuizAttempt(input) {
    return {
      id: `micro-quiz-attempt-${Date.now()}-${input.student_alias}`,
      intervention_id: input.intervention_id,
      topic_id: input.topic_id,
      student_alias: input.student_alias,
      answers: input.answers.map((answer, index) => ({
        question_id: answer.question_id || `question-${index + 1}`,
  ...
  function summariseSubmissions(input) {
  ...
  function levelForMemory(memory) {
    if (["confused", "partially_understood", "ready_to_apply"].includes(memory.current_level)) {
      return memory.current_level;
    }
